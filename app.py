...
# Initialize 'data' to None before its conditional assignment
data = None
...
if uploaded_file is not None:
    ...
    def load_data(file_path, file_type, sheet_name, header_row):
        ...
        return None  # Ensure a None is returned if conditions are not met

    # Only attempt to load data if 'uploaded_file' is not None
...
# Showing value counts of object fields
    ...
    if sub_selected:  # Check if there is a selection
...
if vis_select:
    ...
    try:
        ...
        if walker_html:
            st.components.v1.html(walker_html, width=1100, height=800)  # Adjust width and height as needed
        else:
            st.error("Failed to generate PyGWalker Dashboard.")
    except Exception as e:
        st.error("An error occurred while generating the PyGWalker Dashboard.")
        st.write(e)
